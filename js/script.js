...
let total = 0  //zambyuxi totaly

//zambyuxy cuyc talu hamar
function cardShow(){
...
//producty cuyc talu hamar ekranin
for(let i = 0; i < localStorage.length; i++){

    let product = JSON.parse(localStorage.getItem(`product_${i+1}`)) //qani vor id-n 0 chi karox linel

    if(product != null){  //ete aydpisi apranq chka storgaeum, apa stexcel apranqy
        items.innerHTML += `
        <div class="item">
            <div class="item-img">
...
// Local storage avelacnelu u jnjelu hamar avelordy

function addStorage (item, id){
    let name = item.parentElement.firstElementChild.innerText //itemi parenty dardzel e item info addstorgei hamar
    let price = item.nextElementSibling.firstElementChild.innerText //span inner text
    let img = item.parentElement.previousElementSibling.firstElementChild.getAttribute("src") //src img item-img
    //price, name u image storageum gcelu hamar
    ...
    let newPrice = price * count //producti giny voroshelu hamar qanakov
...
function cardRefresh(){
 ...
 //zambyuxi mej vercrac nuyn apranq tivy hashvely
